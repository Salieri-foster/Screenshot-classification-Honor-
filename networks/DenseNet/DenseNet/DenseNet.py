...
class DenseNet(nn.Module):
    ...
    def forward(self,x ):
        ...
        out=torch.nn.functional.avg_pool2d(features,7, stride=1).view(features.size(0),-1)
...
if __name__ == '__main__':
    ...
    # result=dense(torch.tensor(image).unsqueeze(0).to(device))
    result=dense(image.to(device))
    print(result)
