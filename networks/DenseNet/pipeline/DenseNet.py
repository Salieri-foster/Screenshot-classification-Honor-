...
#计算mean,std
pre_train_set = datasets.ImageFolder('./data/honor/train', transform=transforms.Compose([
    transforms.RandomResizedCrop(128),# 随机剪切成96*96
    transforms.ToTensor()
]))
pre_val_set = datasets.ImageFolder('./data/honor/val', transform=transforms.Compose([
    transforms.RandomResizedCrop(128),# 随机剪切成96*96
    transforms.ToTensor()
]))
...
val_transform = transforms.Compose([
    ...
    transforms.ToTensor(),
    # transforms.Normalize(mean=val_mean,
    #                      std=val_std),
    transforms.Normalize(mean = [ 0.485, 0.456, 0.406 ],
                         std = [ 0.229, 0.224, 0.225 ]),
])
...
#  CIFAR10
# traindir = os.path.join(path, 'data')
# valdir = os.path.join(path, 'data')
#
...
# honor
traindir = os.path.join(path, 'data/honor/train')
valdir = os.path.join(path, 'data/honor/val')

train_set = datasets.ImageFolder(traindir, transform=train_transform)
valid_set = datasets.ImageFolder(valdir, transform=val_transform)
